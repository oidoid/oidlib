...
export function strCapitalize<const T extends string>(str: T): Capitalize<T> {
...
export function strIsBlank(str: string | undefined | null): boolean {
  return str == null || /^\s*$/.test(str)
}

/** Converts all characters of str to lowercase. */
export function strLowercase<const T extends string>(str: T): Lowercase<T> {
  return str.toLocaleLowerCase() as Lowercase<T>
}

/** Converts the first character of str to lowercase. */
export function strUncapitalize<const T extends string>(
  str: T,
): Uncapitalize<T> {
...
export function strUppercase<const T extends string>(str: T): Uppercase<T> {
  return str.toLocaleUpperCase() as Uppercase<T>
}

/** Force English locale when comparing two strings. */
export function strCompareEn(
  lhs: string,
  rhs: string,
