...
export class U32Box implements IntegralBox<U32> {
  ...
  static clamp(x: number, y: number, w: number, h: number): U32Box
  static clamp(xy: Readonly<XY<number>>, wh: Readonly<XY<number>>): U32Box
  static clamp(box: Readonly<Box<number>>): U32Box
  static clamp(
    xXYBox: number | Readonly<XY<number>> | Readonly<Box<number>>,
    yWH?: number | Readonly<XY<number>>,
    w?: number,
    ...
    const box = argsToBox(xXYBox, yWH, w, h)
    return new this(U32XY.clamp(box.x, box.y), U32XY.clamp(box.w, box.h))
  }
  ...
  static fromJSON(json: Readonly<BoxJSON>): U32Box {
    return new this(
      json.xy?.x ?? json.x ?? 0,
      json.xy?.y ?? json.y ?? 0,
      json.wh?.x ?? json.w ?? 0,
  ...
  get centerClamp(): U32XY {
    return U32XY.clamp(this.centerNum)
  }
  ...
  copy(): this {
    return new U32Box(this.x, this.y, this.w, this.h) as this
  }
  ...
  get endClamp(): U32XY {
    return U32XY.clamp(this.endNum)
  }
  ...
  get maxClamp(): U32XY {
    return U32XY.clamp(this.maxNum)
  }
  ...
  get minClamp(): U32XY {
    return U32XY.clamp(this.minNum)
  }
