import { Box, IntegralBox, NumBox, NumXY, U8, U8XY, XY } from '@/oidlib';
...
export class U8Box implements IntegralBox<U8> {
  ...
  static trunc(x: number, y: number, w: number, h: number): U8Box;
  static trunc(xy: Readonly<XY<number>>, wh: Readonly<XY<number>>): U8Box;
  static trunc(box: Readonly<Box<number>>): U8Box;
  static trunc(
    xXYBox: number | Readonly<XY<number>> | Readonly<Box<number>>,
    yWH?: number | Readonly<XY<number>>,
    w?: number,
    ...
    const box = argsToBox(xXYBox, yWH, w, h);
    return new this(U8XY.trunc(box.x, box.y), U8XY.trunc(box.w, box.h));
  }
  ...
  get centerTrunc(): U8XY {
    return U8XY.trunc(this.centerNum);
  }
