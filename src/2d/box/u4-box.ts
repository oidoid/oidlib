...
export class U4Box implements IntegralBox<U4> {
  ...
  static trunc(x: number, y: number, w: number, h: number): U4Box
  static trunc(xy: Readonly<XY<number>>, wh: Readonly<XY<number>>): U4Box
  static trunc(box: Readonly<Box<number>>): U4Box
  static trunc(
    xXYBox: number | Readonly<XY<number>> | Readonly<Box<number>>,
    yWH?: number | Readonly<XY<number>>,
    w?: number,
    ...
    const box = argsToBox(xXYBox, yWH, w, h)
    return new this(U4XY.trunc(box.x, box.y), U4XY.trunc(box.w, box.h))
  }
  ...
  get centerTrunc(): U4XY {
    return U4XY.trunc(this.centerNum)
  }
