...
export class I16Box implements IntegralBox<I16> {
  ...
  static clamp(x: number, y: number, w: number, h: number): I16Box
  static clamp(xy: Readonly<XY<number>>, wh: Readonly<XY<number>>): I16Box
  static clamp(box: Readonly<Box<number>>): I16Box
  static clamp(
    xXYBox: number | Readonly<XY<number>> | Readonly<Box<number>>,
    yWH?: number | Readonly<XY<number>>,
    w?: number,
    ...
    const box = argsToBox(xXYBox, yWH, w, h)
    return new this(I16XY.clamp(box.x, box.y), I16XY.clamp(box.w, box.h))
  }
  ...
  static fromJSON(json: Readonly<BoxJSON>): I16Box {
    return new this(
      json.xy?.x ?? json.x ?? 0,
      json.xy?.y ?? json.y ?? 0,
      json.wh?.x ?? json.w ?? 0,
  ...
  get centerClamp(): I16XY {
    return I16XY.clamp(this.centerNum)
  }
  ...
  get endClamp(): I16XY {
    return I16XY.clamp(this.endNum)
  }
  ...
  get maxClamp(): I16XY {
    return I16XY.clamp(this.maxNum)
  }
  ...
  get minClamp(): I16XY {
    return I16XY.clamp(this.minNum)
  }
