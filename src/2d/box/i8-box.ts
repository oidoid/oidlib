...
export class I8Box implements IntegralBox<I8> {
  ...
  static trunc(x: number, y: number, w: number, h: number): I8Box
  static trunc(xy: Readonly<XY<number>>, wh: Readonly<XY<number>>): I8Box
  static trunc(box: Readonly<Box<number>>): I8Box
  static trunc(
    xXYBox: number | Readonly<XY<number>> | Readonly<Box<number>>,
    yWH?: number | Readonly<XY<number>>,
    w?: number,
    ...
    const box = argsToBox(xXYBox, yWH, w, h)
    return new this(I8XY.trunc(box.x, box.y), I8XY.trunc(box.w, box.h))
  }
  ...
  get centerTrunc(): I8XY {
    return I8XY.trunc(this.centerNum)
  }
